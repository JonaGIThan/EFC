...
const StoreScreen = () => {
  ...
  const fetchProductos = async () => {
    try {
      const response = await fetch('http://172.20.10.7:3000/api/EFC_client/getAllProducts');
      if (!response.ok) {
  ...
  const guardarCambios = async (id, nombre, precio, index) => {
    try {
      ...
      if (nombre.trim() !== productoActual.nombre || parseFloat(precio) !== productoActual.precio) {
        const response = await fetch(`http://172.20.10.7:3000/api/EFC_client/updateProduct/${id}`, {
          method: 'PUT',
          headers: {
