...
const ManagementAccountScreen = () => {
  ...
  const fetchClients = () => {
    fetch('http://192.168.1.103:3000/api/EFC_client/ClientsUpdate')
  ...
  const handleUpdateClient = () => {
    ...
    const formattedStartDate = newStartDateText ? new Date(newStartDateText) : null;
    const formattedEndDate = newEndDateText ? new Date(newEndDateText) : null;
  
    const membershipStatus = newMembershipStatus ? 'Active' : 'Inactive';
  
    fetch(`http://192.168.1.103:3000/api/EFC_client/Update/${id}`, {
      method: 'PUT',
      headers: {
  ...
  const handleClientSelection = (client) => {
    ...
    // Ajustar la lógica para establecer la categoría seleccionada según los valores numéricos recibidos del backend
    switch (client.category) {
      ...
      case 0:
        ...
        break;
      default:
        // Si el valor recibido no coincide con ninguno de los valores esperados, establece una categoría por defecto
        setNewCategory('Student'); // O la categoría que desees por defecto
        break;
    }
