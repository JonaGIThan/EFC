...
const UpdatePersonalScreen = () => {
  ...
  const fetchPersonalList = async () => {
    try {
      const response = await fetch('http://172.20.10.7:3000/api/EFC_client/getAllPersonal');
  ...
  const handleUpdatePersonal = async () => {
    try {
      ...
      const response = await fetch(`http://172.20.10.7:3000/api/EFC_client/updatePersonalInfo/${selectedPersonal.id}`, {
        method: 'PUT',
        headers: {
